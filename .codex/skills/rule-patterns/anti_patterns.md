...
이 문서는 generic 금지 패턴 fallback/reference이며, repo owner docs가 우선한다.

이 문서는 구조를 새로 정의하지 않는다. 구조 정의는 repo owner docs를 우선하고, fallback으로 [`rule-architecture`](../rule-architecture/SKILL.md)를 따른다.

레이어·폴더·의존 기본 규칙은 fallback으로 [`rule-architecture/layers.md`](../rule-architecture/layers.md)를 따른다. 이 문서는 **실제 코드 작성 시 실수하기 쉬운 패턴**만 모은다.
...
`rule-architecture` fallback 규칙을 코드에 적용하는 구체적 패턴이다.
