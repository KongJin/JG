...
이 문서는 generic clean level fallback/reference이며, repo owner docs가 우선한다.

이 문서는 `clean`을 선언할 때 통과해야 하는 검증 게이트의 기본 판정 기준을 정의한다.
구조 규칙은 `rule-architecture`, 금지 패턴은 `rule-patterns` fallback 문서가 다루고, 이 문서는 결과 판정과 검증 순서만 정의한다.
...
> **세부 규칙**: 각 레벨의 포함은 이 문서의 [Clean Levels](#clean-levels)를 참조한다.

> **상위 우선순위**: JG repo에서는 `AGENTS.md` -> `docs/index.md` -> current owner docs 순서를 우선한다.
...
4. Unity 컴파일 확인
5. 필요 시 런타임 스모크 확인
6. **문서·진행 상태 동기화** (`progress.md`, 관련 SSOT, 필요 시 session summary artifact)
7. 최종 상태 기록
...
이 규칙의 의미는 repo owner docs를 우선하고, fallback/reference로는 `rule-architecture`와 `rule-patterns`가 소유한다. 이 문서는 왜 필요한지만 정의한다.

---

## 소유권

* 전역 게이트 fallback: `rule-validation`
