...
| 에디터 MCP 등 자동화 계약 | [unity.md](./unity.md) |
...
엔트리포인트 문서(`../AGENTS.md`, `../docs/index.md` 등)는 **찾아가는 길과 작업 순서만 요약**하고, 규칙 본문은 해당 SSOT에만 둔다.

---

## 작업 레벨별 응집도

- **씬/프리팹**: 요구 오브젝트·참조·순서는 **해당 피처의 Setup/Bootstrap와 실제 serialized contract**에만 남긴다. 같은 내용을 별도 보조 문서로 중복 서술하지 않는다.
...
Plan 모드에서는 **파일 수정, 커밋, 빌드 등 시스템 상태 변경을 금지**한다.
이 모드에서 작업 중 **규칙 문서·기존 코드로 명확한 답이 없으면** 사용자에게 질문한다.
...
- 런타임/스크립트로 씬을 **조용히 고쳐** 실제 씬 계약과 어긋난다 → [anti_patterns.md](anti_patterns.md)의 배선 규칙과 충돌한다.
...
4. 피처 간에는 **포트·이벤트 이름**으로 경계를 두고, 타 피처 문서에 장문으로 "상대 구현"을 적지 않는다.
...
| `AGENTS.md` | 루트 엔트리포인트, 짧은 작업 메모, 다음 시작점 갱신 시 | 코드 작성자 (AI 포함) |

### 최소 업데이트 항목

코드写完 후 아래 세 가지를 먼저 점검한다:

1. **진행 상태** — Phase 완료/진행률, 남은 TODO가 바뀌면 `progress.md` 갱신
2. **설계/결정 근거** — 제품 방향, 범위, 폐기 기능 판단이 바뀌면 `docs/design/` 또는 `docs/discussions/` 갱신
3. **로컬 계약/규칙** — wiring, 직렬화 참조, 규칙 자체가 바뀌면 해당 SSOT 문서 또는 `agent/` 문서 갱신
