# CLAUDE.md

이 문서는 이 레포의 엔트리포인트다.
규칙을 다시 길게 설명하지 않고, 어떤 작업이면 어디로 가야 하는지만 안내한다.

## 먼저 읽기

1. 전역 구조, 레이어, 포트, 씬 계약 체크리스트는 `/agent/architecture.md`에서 시작한다.
2. `Assets/Scripts/Features/<Name>/` 또는 `Assets/Scripts/Shared/`를 수정하면, 해당 `README.md`를 먼저 읽고 수정 후 함께 갱신한다.
3. 구현 중 금지 사항, Bootstrap 예외, runtime lookup 예외 판단은 `/agent/anti_patterns.md`를 따른다.
4. 전역 초기화 순서와 late-join 전제는 `/agent/initialization_order.md`를 따른다.
5. 네트워크 상태 키와 동기화 채널은 `/agent/state_ownership.md`를 따른다.
6. Unity MCP나 에디터 자동화는 `Assets/Editor/UnityMcp/README.md`를 먼저 읽는다.
7. 대규모 기계적 치환 전에는 대상 패턴, 제외 대상, 검증 방법을 먼저 정하고 문서 의미가 바뀌면 README와 규칙 문서도 같이 갱신한다.

## 작업별 진입 경로

| 작업 | 먼저 볼 문서 |
|---|---|
| 피처 경계, 폴더, 레이어, 포트, 네이밍 | `/agent/architecture.md` |
| Bootstrap 책임, EventHandler 위치, runtime lookup 예외 | `/agent/anti_patterns.md` |
| 이벤트 체인 설계, 이벤트 vs 직접 호출 판단 | `/agent/event_rules.md` |
| 전역 초기화 순서, late-join, scene 간 조립 순서 | `/agent/initialization_order.md` |
| CustomProperties 키, 쓰기 소유권, 동기화 채널 선택 | `/agent/state_ownership.md` |
| 로컬 씬 wiring, lookup 예외, 프리팹/씬 계약 | `Assets/Scripts/Features/<Name>/README.md` |
| Shared에 둘 수 있는 것과 없는 것 | `Assets/Scripts/Shared/README.md` |
| Unity MCP 엔드포인트와 테스트 SOP | `Assets/Editor/UnityMcp/README.md` |
| 수치, MVP 기준, 디자인 문장 | `/agent/game_design.md` |
| 문서 소유권과 SSOT 운영 원칙 | `/agent/work_principles.md` |

---

## 충돌 시 우선순위

1. `architecture.md` (folders, features, dependencies, layers, naming, ports)
2. `anti_patterns.md` (금지 패턴, 예외 판단, 리팩터링 교훈)
3. `event_rules.md` (이벤트 체인 방향, 깊이 제한, 이벤트 vs 직접 호출 판단)
4. `initialization_order.md` (전역 피처 초기화 순서와 의존 관계)
5. `state_ownership.md` (CustomProperties 키별 소유권, 동기화 채널 선택)
6. feature `README.md` / `Shared/README.md` (로컬 wiring, lookup 예외, 피처별 계약)
7. `work_principles.md` (문서 운영, SSOT 소유권, 응집도 원칙)
