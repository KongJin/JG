...
| MVP fun validation / playtest | `docs/design/game_design.md`, `docs/playtest/mvp_fun_checklist.md` | MVP 재미 검증, playtest, 재미 체크리스트 | new-skill-candidate | Consider `rule-playtest` only if playtest work repeatedly misses measurement/SSOT routing. |
| Nova1492 resource integration | `docs/plans/nova1492_resource_integration_plan.md` | Nova1492 resource, 원본 리소스, import, staging asset | new-skill-candidate | Keep docs-only for now; create skill only if this becomes a recurring lane. |
...
| Historical discussions | `docs/discussions/*` | rationale, historical, removed features | docs-only | No skill trigger needed. |
