...
| Skill creation / workflow update | `docs/ops/document_management_workflow.md` | skill 생성, description/trigger 변경, workflow 문서 갱신 | covered by `docs/ops/document_management_workflow.md` | skill 생성/변경, workflow 갱신 시 `ops.skill-routing-registry`와 `ops.skill-trigger-matrix` 먼저 보고, 변경이면 `skill trigger checked` 남김 |
...
| Unity scene/prefab/MCP workflow | `docs/ops/unity-ui-authoring-workflow.md`, `tools/unity-mcp/README.md` | Unity, scene, prefab, MCP, Play Mode, compile error, UI authoring | covered by `rule-unity` and `jg-unity-workflow` | Keep both; use `jg-unity-workflow` for JG-specific ordering. |
...
| Nova1492 resource integration | `docs/plans/nova1492-content-residual-plan.md`, `docs/design/world_design.md` | Nova1492 resource, 원본 리소스, import, staging asset | new-skill-candidate | Keep docs-only for now; create skill only if this becomes a recurring lane. |
