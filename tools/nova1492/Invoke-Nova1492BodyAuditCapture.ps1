...
$auditManifestPath = Join-Path $repoRoot "artifacts\nova1492\gx_body_audit_manifest.csv"
$captureRoot = Join-Path $repoRoot "artifacts\nova1492\body-captures"
$contactSheetPath = Join-Path $captureRoot "gx-body-contact-sheet.png"
$manualReviewPath = Join-Path $repoRoot "artifacts\nova1492\gx_body_manual_review.csv"
$captureReportPath = Join-Path $captureRoot "gx-body-capture-report.md"
$menuPath = "Tools/Nova1492/Capture Body Audit Contact Sheet"

. $PSScriptRoot\..\unity-mcp\McpHelpers.ps1

function Invoke-BodyAudit {
    ...
    Write-Host ("[Nova1492BodyAudit] dotnet {0}" -f ($arguments -join " ")) -ForegroundColor Cyan
    & dotnet @arguments
    if ($LASTEXITCODE -ne 0) {
        throw "GxObjConverter body audit failed with exit code $LASTEXITCODE"
    }
}
...
if (-not (Test-Path -LiteralPath $auditManifestPath)) {
    throw "Body audit manifest not found after audit step: $auditManifestPath"
}
...
Write-Host ("[Nova1492BodyAudit] Using Unity MCP bridge: {0}" -f $root) -ForegroundColor Cyan
Write-Host ("[Nova1492BodyAudit] scene={0} playing={1} compiling={2}" -f $health.State.activeScene, $health.State.isPlaying, $health.State.isCompiling) -ForegroundColor DarkGray
...
if (-not (Test-McpResponseSuccess -Response $response)) {
    throw ("Body capture menu failed: {0}" -f ($response | ConvertTo-Json -Compress -Depth 8))
}
...
[PSCustomObject]@{
    ...
    auditManifestPath = "artifacts/nova1492/gx_body_audit_manifest.csv"
    contactSheetPath = "artifacts/nova1492/body-captures/gx-body-contact-sheet.png"
    manualReviewPath = "artifacts/nova1492/gx_body_manual_review.csv"
    captureReportPath = "artifacts/nova1492/body-captures/gx-body-capture-report.md"
