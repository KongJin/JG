...
const defaultPrompts: Record<string, string> = {
  ui_click:
    "short futuristic game UI button click, 0.2 seconds, dry tactile digital tick, clean sci-fi interface, no melody, no vocals, isolated one-shot",
  ui_select:
    "short futuristic UI selection sound, 0.25 seconds, soft bright digital tick with tiny energy lift, no melody, no vocals, isolated one-shot",
  ui_confirm:
    "short futuristic UI confirm sound, 0.45 seconds, compact rising chime with soft mechanical lock, no melody, no vocals, isolated one-shot",
  ui_back:
    "short UI back or close sound, 0.3 seconds, low soft digital fold, restrained sci-fi interface, no melody, no vocals, isolated one-shot",
  ui_error:
    "short restrained UI error beep, 0.35 seconds, tactical soft warning, not harsh, no melody, no vocals, isolated one-shot",
  ui_retry:
    "short network retry UI sound, 0.45 seconds, two clean signal pings, subtle digital scan, no melody, no vocals, isolated one-shot",
  garage_slot_select:
    "short sci-fi garage unit slot select sound, 0.35 seconds, metallic slot click with soft servo, no melody, no vocals, isolated one-shot",
  garage_part_select:
    "short sci-fi garage part selection sound, 0.4 seconds, component scan tick with light mechanical texture, no melody, no vocals, isolated one-shot",
  garage_save:
    "short sci-fi garage save and deploy confirm sound, 0.65 seconds, satisfying mechanical lock-in plus energy pulse, no melody, no vocals, isolated one-shot",
  lobby_ready:
    "short tactical lobby ready confirm sound, 0.5 seconds, confident digital arming cue, clean and restrained, no melody, no vocals, isolated one-shot",
  battle_slot_select:
    "short tactical battle HUD slot select sound, 0.25 seconds, crisp immediate interface tick, subtle combat UI energy, no melody, no vocals, isolated one-shot",
  skill_select:
    "short skill card select sound, 0.4 seconds, clean energy card pickup with light sparkle, no melody, no vocals, isolated one-shot",
};
...
export type AudioProvider = "sandraschi-suno-mcp";
...
export interface SfxManifestItem {
  soundKey: string;
  prompt: string;
...
export interface SfxManifest {
  ...
  createdAt: string;
  updatedAt: string;
  providerDefault: AudioProvider;
  items: SfxManifestItem[];
}
...
export function createDefaultBatch(options: PlanBatchOptions = {}): SfxManifest {
...
export async function readManifest(manifestPath: string): Promise<SfxManifest> {
  const absolutePath = resolveRepoPath(manifestPath);
  const text = await readFile(absolutePath, "utf8");
  const parsed = JSON.parse(text) as SfxManifest;
  validateManifest(parsed);
  return parsed;
}

export async function writeManifest(manifestPath: string, manifest: SfxManifest): Promise<void> {
...
export async function importDownloadedAssets(
  manifest: SfxManifest,
  options: { sourceDir?: string; soundKeys?: string[]; dryRun?: boolean; overwrite?: boolean } = {},
...
export async function syncSoundCatalog(
  manifest: SfxManifest,
  options: { catalogPath?: string; soundKeys?: string[]; dryRun?: boolean } = {},
): Promise<{ results: CatalogSyncResult[]; duplicateKeys: string[] }> {
...
export function validateManifest(manifest: SfxManifest): void {
...
function parseSoundCatalog(text: string): { header: string[]; entries: CatalogEntry[] } {
...
function renderSoundCatalog(header: string[], entries: CatalogEntry[]): string {
...
function matchValue(lines: string[], regex: RegExp): string | undefined {
...
function getDuplicateKeys(entries: CatalogEntry[]): string[] {
...
async function readMetaGuid(metaPath: string): Promise<string> {
...
export function isAllowedAudioTarget(repoPath: string): boolean {
  const normalized = normalizeRepoPath(repoPath);
  return normalized.startsWith("Assets/Audio/UI/") || normalized.startsWith("Assets/Audio/SFX/");
}

async function findAudioFiles(sourceDir: string): Promise<string[]> {
