...
function ok(message: string, structuredContent: Record<string, unknown>) {
  return {
...
function fail(message: string, structuredContent: Record<string, unknown> = {}) {
  return {
    isError: true,
    content: [{ type: "text" as const, text: message }],
    structuredContent,
...
server.registerTool(
  "audio_plan_sfx_batch",
  {
    ...
    description:
      "Create or preview a JG UITK SFX manifest and prompt list for direct sandraschi/suno-mcp generation.",
    inputSchema: {
    ...
    try {
      ...
      return ok(
        writeManifest
          ...
          : `Planned ${manifest.items.length} SFX item(s) without writing a manifest.`,
        { manifestPath, written: writeManifest, items: manifest.items },
      );
    } catch (error) {
...
server.registerTool(
  "audio_import_downloaded_assets",
  {
    ...
    description:
      "Copy audio files downloaded by sandraschi/suno-mcp from an inbox into approved Unity audio folders using manifest sound keys.",
    inputSchema: {
      manifestPath: manifestPathSchema,
...
server.registerTool(
  "audio_sync_sound_catalog",
  {
    ...
    description:
      "Register downloaded audio files in Assets/Data/Sound/SoundCatalog.asset after Unity has generated .meta GUIDs.",
    inputSchema: {
      manifestPath: manifestPathSchema,
...
async function writeManifestFile(manifestPath: string, manifest: Parameters<typeof writeManifest>[1]) {
