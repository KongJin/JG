...
function Get-StitchUnitySurfaceContext {
    ...
    if ([string]::IsNullOrWhiteSpace($MapPath) -and -not [string]::IsNullOrWhiteSpace($SurfaceId)) {
        ...
        throw "Surface '$SurfaceId' does not support 'source freeze -> execution contracts -> UI Toolkit candidate/output' yet. Do not start from stored manifest/map by SurfaceId. Add source-derived execution contract preparation first, or pass -MapPath only for explicit manual inspection/reference."
    }
