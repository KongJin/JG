...
여기서는 `manifest + unity-map + source-derived presentation contract`를 어떤 명령으로 생성/실행하는가만 다룬다.

## 목적

- `screen manifest + unity-map + source-derived presentation contract` 기준으로 surface를 실행한다.
...
기본 실행 단위는 `screen manifest + unity-map + presentation-contract`다.
...
필요할 때만 `Generate-StitchPresentationProfile.ps1 -WriteProfileArtifact`로 디버그 artifact를 남긴다.
...
4. translation
5. `TempScene + SceneView capture` when the surface has a configured review route
6. pipeline result write
...
- 즉 `source -> presentation-contract + manifest/map compile -> translation`이 한 루프로 닫힌다.

source 구조를 읽는 내부 준비 단계가 지원되면:

- `Invoke-StitchSurfaceTranslation.ps1`는 실행 전에 source에서 필요한 execution contract를 다시 준비한다.
- 저장된 manifest/map file은 필요할 때 parity/debug reference로만 남긴다.
...
Set C account-delete-confirm presentation extraction:
...
Set C account-delete-confirm translation:
...
Set C account-delete-confirm review capture only:
...
- 현재 review route가 연결된 surface는 `account-delete-confirm`, `common-error-dialog`다.
...
- `.stitch/contracts/presentations/<surface>.presentation.json`이 존재한다.
