...
* `LobbyBootstrap`는 `LobbyView`, `LobbyPhotonAdapter`, `SceneErrorPresenter`, `SoundPlayer`를 inspector로 받아야 한다.
* `UnitBootstrap`, `GarageBootstrap`는 선택 참조다. 둘 다 있으면 로비 진입 시 함께 초기화하고, 없으면 로비 핵심 흐름만 유지한다.
...
* `GaragePageRoot`가 존재하면 `UnitBootstrap`, `GarageBootstrap`, `GarageNetworkAdapter`도 같은 씬에 함께 존재해야 한다.
