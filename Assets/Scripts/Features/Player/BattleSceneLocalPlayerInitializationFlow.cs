...
{
    internal sealed class BattleSceneLocalPlayerInitializationFlow
    {
        public bool Execute(BattleSceneLocalPlayerInitializationContext context)
        {
...
// csharp-guardrails: allow-null-defense
            ...
            {
                Debug.LogError(
                    "[BattleSceneRoot] WaveSetup is set but CoreObjective is missing. Assign CoreObjectiveSetup on the objective GameObject.");
            }
...
// csharp-guardrails: allow-null-defense
            if (context.CoreObjective == null)
            {
                Debug.LogError("[BattleSceneRoot] Cannot initialize Wave without CoreObjectiveSetup.");
                return false;
            }
        ...
        private static void ValidateAndInitializeSummonSlots(
            BattleSceneLocalPlayerInitializationContext context,
            DomainEntityId playerId,
            UnitSpec[] specs)
        {
            if (specs == null || specs.Length == 0)
            {
                Debug.LogWarning("[BattleSceneRoot] No unit specs for player. Summon slots not initialized.");
                return;
            }
            ...
            {
                Debug.LogWarning(
                    $"[BattleSceneRoot] Player {playerId.Value} starts with insufficient energy " +
...
// csharp-guardrails: allow-null-defense
            ...
            {
                Debug.LogWarning("[BattleSceneRoot] UnitSlotsContainer not assigned. Summon UI skipped.");
                return;
            }
...
// csharp-guardrails: allow-null-defense
            if (placementArea == null)
                Debug.LogWarning("[BattleSceneRoot] PlacementArea not available. Using default spawn position.");
...
// csharp-guardrails: allow-null-defense
    ...
    internal readonly struct BattleSceneLocalPlayerInitializationContext
    {
        public BattleSceneLocalPlayerInitializationContext(
            EventBus eventBus,
            ...
            BattleSceneAudioBootstrapFlow audioBootstrapFlow,
            BattleSceneEndReportingFlow endReportingFlow,
        ...
        public BattleSceneAudioBootstrapFlow AudioBootstrapFlow { get; }
        public BattleSceneEndReportingFlow EndReportingFlow { get; }
