...
{
    public sealed class BattleSceneRoot : MonoBehaviourPunCallbacks
    {
        [Header("Player")]
...
// csharp-guardrails: allow-serialized-field-without-required
        ...
        private readonly BattleSceneGarageBootstrapFlow _garageBootstrapFlow = new();
        private readonly BattleScenePlayerConnector _playerConnector = new();
        private readonly BattleSceneAudioBootstrapFlow _audioBootstrapFlow = new();
        private readonly BattleSceneEndReportingFlow _endReportingFlow = new();
        private readonly BattleSceneLocalPlayerInitializationFlow _localPlayerInitializationFlow = new();
        ...
        private UnitSpec[] InitializeUnitAndGarage(PlayerSetup localPlayerSetup)
        {
            ...
            Debug.Log($"[BattleSceneRoot] Computed {units.Length} unit specs for player {localPlayerSetup.PlayerId.Value}");
            return units;
        }
        ...
        {
            _localPlayerInitializationFlow.Execute(new BattleSceneLocalPlayerInitializationContext(
                _eventBus,
                _disposables,
                _localPlayerSetup,
                _camera,
...
// csharp-guardrails: allow-null-defense
            ...
            {
                if (_eventBusConsumers[i] is IBattleSceneEventBusConsumer consumer)
