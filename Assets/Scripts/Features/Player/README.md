...
- 이 피처의 초기화 순서와 remote wiring 전제: 이 문서의 `## 로컬 계약`
- Player `CustomProperties` 소유권과 동기화 채널: 이 문서의 `## 네트워크 동기화`

## 이 피처의 책임

- 플레이어 스폰 (Photon Instantiate)
- 플레이어 위치/회전 네트워크 동기화 (시점용 카메라 추적 포함)
- 로컬/원격 플레이어 분기 초기화
...
- Energy 리소스 관리 (EnergyAdapter → IUnitEnergyPort 구현 — 유닛 소환 전용 자원)
- Energy 리젠 틱 (EnergyRegenTicker, Presentation thin shell)
- Energy UI (EnergyBarView, 로컬 플레이어 전용 스크린 HUD)

> **기획 변경 참고**: game_design.md에 따라 플레이어는 직접 조작 아바타가 아닌 **차고 소유자, 전투 중 소환자, 빌드 책임자**로 재정의됨. 전투 중 기본 액션은 유닛 슬롯 확인 → 유닛 선택 → 배치 구역 지정 → 소환 타이밍 판단. 기존 이동/점프 입력 코드는 잔존하지만 신규 게임 루프에서 비활성 상태.
...
| 데이터 | 방식 | 용도 |
|---|---|---|
| 위치, 회전 | `OnPhotonSerializeView` (연속 데이터) | 매 프레임 보간 (시점용) |
...
| Energy | `CustomProperties` (상태 동기화) | 유닛 소환 자원, late-join 대응 |
...
- **Domain**: `Player` (Energy 필드 포함 — Phase 3에서 Mana → Energy 명칭 변경 예정: `MaxEnergy`, `CurrentEnergy`, `SpendEnergy()`, `RegenEnergy()`; LifeState: `Alive`/`Downed`/`Dead`, `TakeDamage()` → Downed 전이, `TickBleedout()`, `ForceDowned()` (late-join 상태 복원용), `Rescue()`, `TickInvulnerability()` (시간 경과 후 무적 해제), `Hydrate(float hp, float energy)` (late-join/CustomProperties 상태 복원용, HP/Energy를 도메인에 직접 반영), `Die()`), `PlayerSpec` (Defense, MaxEnergy, EnergyRegenPerSecond 필드 포함), `MovementRule`, `LifeState`, `BleedoutRule` (Duration=10s), `RescueRule` (ChannelDuration=1.5s, HpPercent=50%, EnergyPercent=50%, InvulnerabilityDuration=2s, MaxRange=3f)
- **Application**: `PlayerUseCases` (구조 메서드: `CompleteRescue()`는 rescuer alive + target downed 재검증 후 RPC 전송, `SpawnRemote()` static factory로 원격 플레이어 도메인 생성 통합, `FindRescueTarget()` 쿼리 포함, 이벤트 구독으로 SyncLifeState 네트워크 전송), `PlayerNetworkEventHandler` (Health/Energy CustomProperties 수신 시 `Player.Hydrate()`로 도메인 상태도 갱신, LifeState CustomProperties 수신 처리 — `LifeState.Alive`는 `Player.Respawn()` + 이벤트 발행으로 late-join 클라이언트의 다운/사망 상태 복구, `HandleRemoteRescued`는 target.IsDowned 가드 포함, `IPlayerLookupPort`로 targetId 기준 도메인 플레이어 lookup), `PlayerDamageEventHandler` (PlayerDownedEvent/PlayerDiedEvent 분기 발행, downed/death dedupe 플래그), `BleedoutTracker` (Downed 상태 틱, bleedout 만료 시 PlayerDiedEvent 발행), `RescueChannelTracker` (채널링 진행/완료/취소 + 이벤트 기반 자동 취소: rescuer downed/died 또는 target died/rescued 시 ForceCancel → 네트워크 취소 전송 + RescueChannelCancelledEvent 발행), `InvulnerabilityTracker` (Rescue 후 무적 시간 틱, 만료 시 자동 해제), `GameEndEventHandler`, `EnergyAdapter` (`IUnitEnergyPort` 구현, Energy 차감/리젠/네트워크 동기화 — 유닛 소환 자원), 이벤트(`PlayerHealthChangedEvent`, `PlayerDiedEvent`, `PlayerDownedEvent`, `PlayerRescuedEvent`, `RescueChannelStartedEvent`, `RescueChannelCancelledEvent`, `PlayerRespawnedEvent`, `PlayerEnergyChangedEvent`, `GameEndEvent`), 포트(`IPlayerNetworkCommandPort` (SyncLifeState, SendRescue, SendRescueChannelStart/Cancel 추가), `IPlayerNetworkCallbackPort` (OnLifeStateSynced, OnRemoteRescued, OnRemoteRescueChannelStarted/Cancelled 추가), `IPlayerLookupPort` (플레이어 ID → Domain.Player + 위치 조회), `ISpeedModifierPort`, `IUnitEnergyPort`)
- **Infrastructure**: `PlayerMotorAdapter`, `PlayerNetworkAdapter` (`MonoBehaviourPunCallbacks`, HP/Energy/LifeState CustomProperties 송수신 (Phase 3: Energy는 기존 마나 키 재사용), Rescue/RescueChannel RPC, `HydrateFromProperties()` late-join 상태 복원 (LifeState enum 범위 검증 포함), RPC 수신 시 파라미터 검증 — null/empty string, NaN/음수 damage, enum 범위 체크, optional attacker/killer ID는 `default(DomainEntityId)`로 전달), `PlayerCombatTargetProvider` (Combat의 `ICombatTargetProvider` 구현, Downed 상태에서 추가 피해 무시), `EntityAffiliationAdapter` (Combat의 `IEntityAffiliationPort` 구현, ID 프리픽스 기반 소속 판정), `PlayerLookupAdapter` (`IPlayerLookupPort` 구현, PlayerSceneRegistry 래핑), `EnergyRegenAdapter` (`IUnitEnergyPort` 구현 — Energy 차감/리젠)
- **Presentation**: `PlayerInputHandler` (Interact 입력으로 `PlayerUseCases.FindRescueTarget()` 호출하여 구조 시작/취소, `PlayerRespawnedEvent` 구독으로 respawn 시 입력 복구), `PlayerView` (리모트 컴포넌트 비활성화는 PlayerSetup이 담당, View는 이벤트 구독 분기만 수행), `PlayerHealthHudView`, `EnergyBarView` (로컬 전용 스크린 HUD — 기존 ManaBarView 리네임), `EnergyRegenTicker` (thin Update shell → EnergyAdapter.TickRegen), `BleedoutTicker` (thin Update shell → BleedoutTracker.Tick), `RescueChannelTicker` (thin Update shell → RescueChannelTracker.Tick → 완료 시 CompleteRescue), `InvulnerabilityTicker` (thin Update shell → InvulnerabilityTracker.Tick), `DownedOverlayView` (다운 시 bleedout 타이머바 + 구조 채널링 진행 바, BleedoutTracker.Elapsed / RescueChannelTracker.Elapsed를 직접 읽어 dual state 방지), `CameraFollower`
