...
Skill 피처는 스킬 입력, 덱 순환, 시작 스킬 선택, 네트워크 캐스트 이벤트 해석을 담당한다.

## 먼저 읽을 규칙

- 전역 구조, 레이어, 포트 위치: [architecture.md](../../../../agent/architecture.md)
- Bootstrap 책임, EventHandler 위치, Unity 타입 포트 규칙: [anti_patterns.md](../../../../agent/anti_patterns.md)
- 이 피처의 초기화 순서와 Skill 이후 Wave wiring 전제: 이 문서의 `## 2단계 초기화 흐름`
- `skillsReady` 소유권과 동기화 채널: 이 문서의 `### CustomProperties 소유권`
...
- **덱 다음 장 미리보기 (MVP ①)**: `Deck.PeekNextDrawSkillId()`로 뽑기 더미 맨 위(다음 `Draw`와 동일)를 읽고, `DeckCycleHandler`가 시전 후·초기화 직후 `DeckNextDrawPreviewEvent`를 발행한다. `BarView`는 선택 필드 `nextDrawPreviewIcon` / `nextDrawHintLabel`이 연결된 경우에만 아이콘·라벨을 갱신한다(미연결 시에도 이벤트는 발행됨).
...
- **SkillsReady CustomProperty (Phase 3 제거)**: Phase 3 이전에는 시작 스킬 선택 완료 시 `SkillNetworkAdapter.SyncSkillsReady()`로 설정되고 Wave 피처가 읽어 전원 준비를 확인했다. Phase 3에서 GameScene 진입 시 바로 게임 시작하므로 `skillsReady`는 더 이상 사용되지 않는다. 코드는 유지되지만 호환성 목적이며, 신규 게임 루프에서는 불필요.

## 로컬 계약

- Skill 초기화는 `InitializePreSelection`과 `InitializePostSelection`의 2단계로만 진행한다.
- 스킬 연출은 로컬 직행이 아니라 RPC를 한 번 타고 돌아온 결과 이벤트를 기준으로 움직인다.

### CustomProperties 소유권

- `skillsReady`는 Phase 3에서 더 이상 사용되지 않음 (GameScene 진입 시 바로 시작). 코드는 호환성 유지.
- Wave를 포함한 다른 피처는 `skillsReady`를 읽지 않음.
...
현재 구현은 로컬에서도 RPC를 한 번 타고 돌아온 결과를 이벤트로 해석한다.
즉 연출과 후속 피처 연결은 `SkillNetworkEventHandler`가 발행한 이벤트를 기준으로 움직인다.
...
StartSkillSelectedEvent 수신
  └── StartSkillSelectionHandler → InitializePostSelection(chosenIds, onComplete)
        ...
        ├── Equip 2 slots
        ├── DeckCycleHandler (+ 초기 `PublishDeckPreview()`), CastSkillUseCase, SlotInputHandler
        ├── SkillRewardAdapter
        └── onComplete()
              └── WaveBootstrap.Initialize(...)  ← Phase 3: 더 이상 skillsReady 배리어 없음, 바로 시작
```

## 데이터 드리븐 구조

스킬 분류 태그와 현재 코드 필드의 매핑은 이 문서의 `## 데이터 드리븐 구조`와 `SkillGameplayTags` 정의를 기준으로 본다.
...
- `SkillSetup` (피처 루트에 위치)
  ...
  - 단, `_startSkillSelectionView`는 별도 `StartSkillSelectionCanvas` 씬 인스턴스를 가리키는 scene-only 참조라서 `[SerializeField]`만 사용한다. prefab asset에서는 비워 두고, 각 씬 인스턴스에서 반드시 연결해야 한다.
  ...
  - ~~`SkillNetworkAdapter.SyncSkillsReady()` 호출~~ (Phase 3에서 제거 — 더 이상 사용 안 함)
  - `SwapSkill(slotIndex, skillId)`: 런타임 스킬 교체 API
  - `ISkillRewardPort SkillReward`, `ISkillIconPort SkillIcon`, `ISkillUpgradeQueryPort SkillUpgradeQuery`, `ISkillUpgradeCommandPort SkillUpgradeCommand` 프로퍼티를 노출하여 외부(GameSceneRoot → WaveBootstrap)에서 포트 인터페이스로 사용
  - `JG_GameScene`의 월드 스킬 시스템은 `GameSceneRoot/SkillWorldSystems` 아래에 두고, `ProjectileSpawner`와 `ZoneSetup`은 `GameSceneRoot`가 직접 참조한다.
  - `SkillBarCanvas`와 `StartSkillSelectionCanvas`는 최상위 `UIRoot` 아래에 두고, `SkillSetup`은 이 새 경로 기준 씬 참조를 유지한다.
...
- `DeckCycleHandler`
  - `SkillCastedEvent` 구독 → 시전된 스킬을 덱에 버리기 → 다음 스킬 드로우 → 같은 슬롯에 재장착 → `DeckNextDrawPreviewEvent` 발행
  - `IEventPublisher` 주입. 초기 HUD 동기화용 `PublishDeckPreview()` (SkillSetup이 장착 직후 1회 호출)
...
- `SkillNetworkAdapter` (`MonoBehaviourPun`) — RPC 송수신. 페이로드 끝에 `GameplayTags`(uint) 포함; 짧은 구버전 페이로드는 `Infer`로 태그 복원. `TryDeserialize` 패턴으로 payload 검증 수행 (문자열 null 체크, float NaN/Infinity, enum 범위, slotIndex 범위 `[0, SkillBar.SlotCount)`, 최소 길이). ~~`SyncSkillsReady()`~~ (Phase 3에서 제거 — 더 이상 사용 안 함). ~~`IsPlayerSkillsReady(player)`~~ (마찬가지로 제거).
...
- `BarView` — `SkillEquippedEvent`로 슬롯 아이콘 표시, `DeckNextDrawPreviewEvent`로 선택적 **다음 드로우** 아이콘/라벨 갱신(`nextDrawPreviewIcon`, `nextDrawHintLabel` 미할당 시 UI 변경 없음)
...
- `UIRoot/SkillBarCanvas` 프리팹 인스턴스
- `UIRoot/StartSkillSelectionCanvas` 프리팹 인스턴스 (시작 시 비활성, 8개 `SkillSelectButton` 미리 배치)
- 씬 루트 오브젝트 `GameSceneRoot` (`WaveSystems` / `StatusSystems` / `PlayerSceneTickers` / `CombatSystems` / `SkillWorldSystems` 자식 포함)
- `GameSceneRoot._skillSetup` 필드에 `SkillSetup` 참조
- `GameSceneRoot._projectileSpawner` 필드에 `ProjectileSpawner` 참조
...
- `SkillSetup._startSkillSelectionView` — `StartSkillSelectionView` 참조 (시작 스킬 선택 패널). `SkillBarCanvas` prefab asset이 아니라 각 씬의 prefab instance override에서 연결한다.
...
- `BarView.nextDrawPreviewIcon` / `nextDrawHintLabel` — 덱 **다음 드로우** 미리보기. `SkillBarCanvas`(또는 씬 인스턴스)에서 UI 오브젝트를 추가·연결하면 활성화된다. 비워 두면 동작에 필수는 아니다.

#### 런타임 생성 오브젝트

- 없음 (시작 스킬 선택 UI는 프리팹에 미리 배치)

#### MCP 자동화 (`JG_GameScene`)

- `UIRoot/StartSkillSelectionCanvas/Panel/ButtonGrid/SkillButton0` … `SkillButton7` — 각 오브젝트에 `UnityEngine.UI.Button`이 붙어 있음; `POST /ui/button/invoke`로 토글 선택
- `UIRoot/StartSkillSelectionCanvas/Panel/ConfirmButton` — 2개 선택 후 `onClick`으로 `StartSkillSelectedEvent` 발행
- 경로 SSOT와 invoke helper는 `tools/mcp-test-common.ps1`의 `Game.StartSkillButton0`, `Game.StartSkillButton1`, `Game.StartSkillConfirm`를 따른다
- 로비→게임까지 포함한 스모크: `tools/mcp-test-lobby-scene.ps1 -CompleteGameSceneStartSkills`
...
- `GameSceneRoot`가 `SkillSetup.InitializePreSelection(...)`을 호출하므로 스킬 선택 UI가 표시된다
- `InitializePostSelection`이 호출되지 않으면 입력 바인딩이 없어 스킬 사용 불가
- Inspector에서 `SkillCatalogData`를 연결해야 한다 — 없으면 초기화 중단
- `_startSkillSelectionView`가 연결되지 않으면 `InitializePreSelection`이 즉시 에러를 로그하고 중단한다. `JG_GameScene` 등 게임 씬 인스턴스 override에서 반드시 연결해야 한다.
...
- `Wave`: `ISkillRewardPort` (보상 풀 제공), `ISkillUpgradeCommandPort` (보상 강화 적용). Phase 3 이전에는 `SkillsReady` CustomProperty로 Wave 시작 배리어 역할을 했으나 제거됨.
