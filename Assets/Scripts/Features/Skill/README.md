...
- `StartSkillSelectionCanvas/Panel/ButtonGrid/SkillButton0` … `SkillButton7` — 각 오브젝트에 `UnityEngine.UI.Button`이 붙어 있음; `POST /ui/button/invoke`로 토글 선택
- `StartSkillSelectionCanvas/Panel/ConfirmButton` — 2개 선택 후 `onClick`으로 `StartSkillSelectedEvent` 발행
