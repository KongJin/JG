...
- `skillsReady`는 Skill만 쓰며, 다른 피처는 읽기만 허용된다.

### CustomProperties 소유권

- `skillsReady`는 Skill만 쓴다.
- Wave를 포함한 다른 피처는 `skillsReady`를 읽기만 한다.
- 시작 스킬 선택 완료 전에는 `false` 또는 미설정 상태일 수 있고, 선택 완료 후 `SkillNetworkAdapter.SyncSkillsReady()`가 authoritative write를 수행한다.
