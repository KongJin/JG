...
이 모드에서 작업 중 **의문이 생기면 즉시 사용자에게 질문**한다.

**규칙:**
- 가정하지 말고 **확인**한다
- "이렇게 하면 될 것 같다" → **"이렇게 해도 될까요?"** 라고 묻는다
- 불확실한 선택지가 있으면 **질문 도구**로 사용자에게 선택권을 준다
- 문맥 파악 중이라도 **의심스러운 부분은 건너뛰지 않고 질문**한다

**예시:**
- ❌ "3D 모델이 없으니 아이콘 카드로 대체하겠습니다" (가정)
- ✅ "3D 모델이 없습니다. 아이콘 카드로 대체할까요, 아니면 3D 뷰포트 구조만 준비할까요?" (질문)
...
| `QWEN.md` | 세션 종료 시 작업 요약, 다음 세션 시작점 갱신 (세션 메모 전용, 공식 진행률 SSOT 아님) | AI 에이전트 (세션 종료 직전) |
...
3. **로컬 계약/규칙** — wiring, 직렬화 참조, 규칙 자체가 바뀌면 해당 README 또는 `agent/` 문서 갱신
