...
### MCP 캡처 결과 (2026-04-17)

참고 캡처: [`garage-screen-capture.png`](../../artifacts/unity/garage-screen-capture.png)

**새로 확인된 포인트:**
- `AccountCard`는 이제 `display name / Google / Logout / Delete Account / status`까지 노출되지만, 현재는 흰색 블록처럼 보이는 영역과 텍스트 밀도 불균형으로 카드 완성도가 낮음
- 좌측 `Rooms` 패널은 라벨과 입력창이 너무 가까워 placeholder와 시선이 겹쳐 보임
- 중앙 상단 프리뷰 영역은 비어 있는 검은 사각형처럼 보여 "미완성" 인상을 줌
- 우측 상태 카드(`Ready window open`, stats)는 정보량 대비 폭이 좁아 가독성이 떨어짐
- 전체 구조는 유지 가능하지만, 현재 병목은 "새 컴포넌트 부족"보다 "기존 카드의 여백/밀도/빈 상태 처리" 쪽에 있음
...
5. **장식보다 조형 품질 우선** — `canvas-design` 식의 무드 차용은 가능하지만, Garage는 상호작용 대시보드로서 정보 구조와 상태 표현을 우선한다
...
- [ ] 현재 `Logout`, `Delete Account` 추가 이후 버튼군의 세로 밀도와 상단 텍스트 충돌 재정리
- [ ] 흰색 블록처럼 보이는 상단 영역 원인 확인 및 제거
...
- [ ] 프리뷰 비활성/미로드 상태 placeholder 설계 - 검은 빈 사각형으로 보이지 않게
...
| Phase | 상태 | 시작일 | 완료일 | 비고 |
|---|---|---|---|---|
| Phase 1: 치명적 문제 | ✅ 완료 | 2026-04-13 | 2026-04-14 | 1.1~1.3 MCP scene 정리 + 코드 개선 완료 |
| Phase 2: 상호작용 | ✅ 완료 | 2026-04-14 | 2026-04-14 | 2.1~2.4 코드 개선 완료 |
| Phase 3: 디자인 시스템 | ✅ 완료 | 2026-04-14 | 2026-04-14 | 3.1 ButtonStyles, 3.2 ThemeColors, 3.3 색상 토큰, 3.4 계정 카드(MCP) 완료 |
| Phase 4: 고급 개선 | 🟡 부분 완료 | 2026-04-14 | - | 4.1 Preview 분리, 4.2 비교 툴팁, 4.3 키보드 단축키 완료. 4.4 모바일 보류 |
