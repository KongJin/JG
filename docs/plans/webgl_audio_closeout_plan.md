...
Account/Garage account acceptance, Firebase Auth/Firestore, Google linking은 별도 WebGL account owner가 소유한다.
`SoundPlayer` runtime host/template residual은 runtime contract guard owner가 소유한다.
...
- SoundPlayer AudioSource/template residual은 technical debt recurrence owner에서 판단한다.
