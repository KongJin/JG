...
- 2026-05-02: profile metadata를 `NovaPartAlignmentCatalog.asset`에 promotion했고, direct EditMode tests로 인간형 shell profile과 high-risk leg queue metadata를 확인했다.
...
현재 Garage preview는 `하단(기동) / 중단(프레임) / 상단(무장)` 세 부품을 모두 보여줄 수 있어도, 일부 조합에서 위치가 규칙적으로 맞지 않는다.
특히 인간형 무장은 XFI에 명시 transform이 거의 없고, 원본 클라이언트 내부의 슬롯명과 slot-specific GX 파일 흔적으로 볼 때 원본 조립은 단순한 mesh center 정렬보다 더 강한 per-slot 또는 per-part 규칙을 사용했을 가능성이 높다.
...
| 인간형 무장은 프레임이 무장 shell 안으로 들어가는 방식이다 | 무장 중앙 공간이 비어 있고 프레임 bounds center가 그 안에 들어가야 자연스럽다 | 인간형 무장 단품 bounds와 조립 capture 비교 |
...
| `ReviewOnlyHumanoidShellBoundsCenter` | 인간형 무장 shell 가설을 검토하기 위한 capture/review 후보. `match` 승인 전에는 정상 preview anchor로 쓰지 않는다 |
| `FrontAddon` | 전면 보조/센서형 부품 anchor |
| `ManualOffset` | 원본 evidence나 visual review로 확정된 부품별 예외 |
| `Disabled` | 카탈로그에서는 유지하지만 조립 preview에서 제외하거나 후속 조사로 넘기는 상태 |
...
- `pending/review/unsure`, direction-only XFI, review-only anchor는 정상 Garage preview 성공으로 렌더하지 않고 explicit unavailable/review-required state 또는 capture-only evidence로 분리한다.
...
2. Profile seed generation: 사용자-confirmed 형태 mapping을 우선하고, direction-only 인간형 무장은 `ReviewOnlyHumanoidShellBoundsCenter` 또는 `Disabled` 후보로 두되 `review` confidence로 남기고 정상 preview `auto_ok`로 promotion하지 않는다.
...
초기 queue는 인간형 무장 shell(스핏파이어, 해머쇼크, 핸드캐넌), 어깨형 pair, 탑형 top, 그리고 단품 GX 품질이 의심되는 하단(델피누스, 캐터필러, 피파울, 옵테릭스)이다.
...
- Garage preview direct tests: valid form 조합 profile anchor 적용, invalid form 조합 차단, direction-only/review/pending 조합 fail-closed, fallback 사용 report 기록
...
- `direction-only`, `pending`, `review`, `unsure` profile은 normal preview success가 아니다. fail-closed 상태로 남기고, 깨진 조립을 bounds fallback으로 그럴듯하게 보여주지 않는다.
