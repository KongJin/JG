...
- Decision owner: WebGL build를 브라우저에서 실행하고 console/audio state를 확인할 수 있는 smoke executor.
- Next action: Audio load, playback unlock, settings persistence, runtime audio contract를 같은 WebGL build 기준으로 실행하고 각 항목을 `success`, `blocked`, `mismatch`로 기록한다.
- Stale handling: WebGL build/browser smoke를 이번 pass에서 실행하지 못하면 product success로 올리지 않고, 실행 환경 blocker와 다음 확보 방법만 남긴 뒤 active 유지 여부를 재검토한다.
