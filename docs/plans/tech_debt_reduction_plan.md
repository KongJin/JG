...
이 문서는 프로젝트의 기술부채를 "무엇이 불편한가" 수준이 아니라, **무엇을 어떤 순서로 줄여야 배포 신뢰도가 올라가는가** 기준으로 정리한 reference다.
현재 실행 상태와 우선순위는 [`progress.md`](./progress.md)를 우선한다.

작성 당시 핵심 방향은 아래 세 가지였다.
...
- 공식 진행 상태는 [`progress.md`](./progress.md)를 우선한다.
- 이 문서는 당시 우선순위와 실행 순서를 설명하는 reference다.
- 현재 우선순위 판단은 `progress.md`를 따른다.
