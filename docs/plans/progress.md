...
- Phase 0~9의 `완료` 표기는 주로 code path 기준이다. 2026-04-26 single-client smoke에서 `LobbyScene -> BattleScene`, 적 스폰/제거, core damage -> defeat overlay까지 에러 0으로 재현됐다. 같은 날 actual UI path로 Lobby room create -> room detail -> ready -> start -> BattleScene 진입, actual placement/summon stat smoke, diagnostic victory result smoke도 통과했다. 2026-04-27에는 current UI hierarchy 기준 placement path, natural final-wave victory, defeat regression smoke가 모두 `newErrorCount: 0`으로 통과했다. 실제 플로우 acceptance는 [`game_scene_flow_validation_closeout_plan.md`](./game_scene_flow_validation_closeout_plan.md)가 소유하며, 남은 리스크는 summon failure rollback/enemy priority EditMode 실행 확인, placement automation, Phase 5 멀티플레이어 smoke다.
...
- `GameScene` BattleScene closeout 우선순위: summon rollback / enemy priority direct tests 실행 확인 -> placement automation -> 2-client sync / late-join -> mobile HUD/input framing
...
| `GameScene` flow closeout | summon failure rollback과 enemy target priority direct test asset은 추가됐고 compile-clean은 통과. EditMode 실행은 open-editor-owned project로 blocked이며, 다음은 실행 확인 후 placement automation, 2-client sync/late-join, GameEnd result HUD actual player-flow checklist 갱신 |
...
- `GameScene` 쪽은 [`game_scene_flow_validation_closeout_plan.md`](./game_scene_flow_validation_closeout_plan.md)에 따라 single-client actual UI path/victory/defeat pass를 기준선으로 두고, summon rollback/enemy priority EditMode 실행 확인, placement automation, 2-client sync를 success/blocked/mismatch로 분리해 닫는다.
