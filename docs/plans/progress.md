...
- Technical debt recurrence prevention gate는 2026-04-29 기준 baseline lock, worktree split, raw lookup exception 분류, actual acceptance 분리, active plan lifecycle audit를 [`technical_debt_recurrence_prevention_plan.md`](./technical_debt_recurrence_prevention_plan.md)에 기록했다. 이후 PlayerSpec/EnemyData/SoundPlayer runtime `Resources.Load` fallback을 serialized contract로 옮기고 runtime feedback prefab을 `Assets/Prefabs/RuntimeFeedback/`으로 이동했다. 이 게이트는 `clean` 선언을 대체하지 않고 compile/static/docs/asset 검증과 runtime/WebGL/2-client evidence 분리를 유지하는 기준이다.
...
- `GameScene` actual-flow closeout: direct EditMode 실행 확인은 `open-editor-owns-project` blocked, drag/drop residual, 2-client sync/late-join hydration
...
- `Account/Connection UI` runtime: Lobby shell visibility는 통과, WebGL/account/cloud product acceptance는 별도 pass
...
| `Non-Stitch UI migration` | Account/Sync와 Connection/Reconnect는 UI Toolkit candidate surface, preview evidence, Lobby runtime shell visibility까지 확보. WebGL/account/cloud acceptance는 shared `Account/Garage` lane에 남기고, 남은 native/mixed 후보는 Stitch accepted baseline부터 시작한다 |
| `Prefab management` | closeout 완료. `PlayerHealthHudView`, `EnemyHealthBar`, `DamageNumber`는 `Assets/Prefabs/RuntimeFeedback/` compatibility surface로 이동. 남은 판단은 필요 시 world-space feedback visual consistency micro-surface 여부다 |
...
- Prefab 관리 빈틈은 inventory, approval manifest, review/import stale path fail, missing prefab fallback 차단, override drift report, Resources migration 연결까지 닫혔다. Runtime feedback prefab은 `Assets/Prefabs/RuntimeFeedback/` compatibility surface로 분리됐고, screen UI replacement 후보와 섞지 않는다.
