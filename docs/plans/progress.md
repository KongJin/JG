...
- Stitch-to-Unity는 UI Toolkit candidate surface 루트를 기준으로 한다. SetB Garage UITK pilot은 source hierarchy, capture, scoped policy evidence가 생겼고, Account Settings overlay는 새로 열 때 UI Toolkit candidate로 다시 가져온다.
- 새 UX/UI는 Stitch source freeze -> UI Toolkit candidate surface -> preview capture/report를 기본 루트로 삼는다. 기존 runtime UI script는 현재 scene/prefab 호환성 범위로만 다룬다.
- `Set B Garage`는 visual fidelity final judgment가 직접 residual이다. `Operation Memory`는 2026-04-27 Stitch visual candidate가 생겼지만 CTA/copy/list structure 수정 전에는 accepted source로 보지 않는다. 두 화면의 visual 개선과 source 후보 판단은 [`set_b_operation_memory_visual_improvement_plan.md`](./set_b_operation_memory_visual_improvement_plan.md)가 소유한다. Garage save/load, settings, accessibility는 shared `Account/Garage` validation lane에서 본다.
...
- `Set B / Operation Memory` visual 개선과 source 후보 판단
- `Shared UI` Stitch 후보 관리: 공용 UI는 Shell/Navigation, Feedback/Status, Components/Controls split source 기준으로 관리하고, Garage/Operation Memory는 shared shell만 참고하며 page CTA는 각 화면이 소유
- `Non-Stitch UI` migration: Stitch source freeze가 없는 Unity-native/mixed UI를 Stitch에서 다시 만든 뒤 UI Toolkit candidate surface로 가져오는 실행 계획 정착
- shared `Account/Garage` WebGL save/load, settings interaction, save action accessibility 검증
- Stitch generic source-to-contract route 정착과 UI Toolkit candidate import 판단
...
| `Set B / Operation Memory visual` | Set B Garage final visual fidelity와 Operation Memory CTA/copy/recent-5 structure 개선 후 source 후보 판단 |
| `Non-Stitch UI migration` | 다음 native/mixed 후보는 Stitch accepted baseline을 만든 뒤 UI Toolkit candidate surface로 가져온다 |
...
- Lobby/Garage 및 Operation Memory 쪽은 [`set_b_operation_memory_visual_improvement_plan.md`](./set_b_operation_memory_visual_improvement_plan.md)에 따라 Set B Garage visual fidelity와 Operation Memory source 후보 판단을 먼저 닫는다.
- shared `Account/Garage` lane에서는 Garage save/load WebGL, settings interaction, save action 접근성을 계속 추적한다.
- Stitch lane 쪽은 `Set A/B/C/D/E`와 추가 `GameScene HUD` source freeze를 generic onboarding 기준 샘플로 삼아, 이후 다시 여는 inventory set을 per-surface script edit 없이 UI Toolkit candidate 루프의 verdict까지 태울 수 있게 source facts/draft/validate route를 일반화한다.
