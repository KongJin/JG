...
- Unity 프로젝트 내부에는 첫 staging 후보 40개만 `Assets/Art/Nova1492/` 아래로 복사했다.
...
- 아직 prefab 적용과 WebGL 로드 검증은 하지 않았다.
...
- Phase 0~2는 완료됐고, Phase 3 이후는 `progress.md`가 다시 현재 포커스로 올릴 때 재개하는 reference 상태다.
...
- `.GX`, `.xfi`, `.dat` 변환기를 이 계획 안에서 구현하는 작업
...
- `.GX`, `.xfi`, `.dat`은 바로 runtime에 넣지 않고 포맷 조사 대상으로 분리한다.
...
| Unity 경로 | 내용 |
|---|---|
| `Assets/Art/Nova1492/UI/` | 로비, Garage, 버튼, 패널, 스킬/부품 아이콘 |
| `Assets/Art/Nova1492/Units/` | 유닛/파츠/실루엣 스프라이트 |
| `Assets/Art/Nova1492/Effects/` | 미사일, 폭발, 전투 이펙트 후보 |
...
| `.GX/.xfi` 포맷 미해석 | 즉시 적용 대상에서 제외하고 조사 phase로 분리 |
