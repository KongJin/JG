...
> role: reference
> owner_scope: Lobby, Garage, BattleScene 진입과 전투 루프의 수동 runtime validation checklist template, BattleScene multiplayer residual closeout checklist
...
BattleScene 2-client multiplayer residual은 별도 active plan을 만들기 전까지 이 checklist가 current owner로 맡는다. 실제 실행 evidence나 artifact 전용 소유자가 열리면 그 plan 또는 artifact로 넘긴다.
...
- 이 문서는 현재 `BattleScene / Multiplayer Sync` residual의 checklist owner다.
...
| BattleScene blocker owner 분리 | 실패 시 runtime event/state 문제인지 HUD/input/prefab 문제인지 메모에 분리한다. | | |
