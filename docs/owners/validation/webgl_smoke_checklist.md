...
> role: reference
> owner_scope: WebGL 수동 smoke 절차와 기대 결과, WebGL Account/Garage product smoke residual closeout checklist
> upstream: docs.index, ops.firebase-hosting, ops.acceptance-reporting-guardrails
> artifacts: none
>
> 생성일: 2026-04-17
> active closeout owner: this checklist for WebGL Account/Garage product smoke residual; no separate WebGL account active plan currently exists

이 문서는 Phase 10/11의 WebGL 실기 검증을 위한 **수동 smoke 체크리스트**다.
Active residual과 closeout 판정은 별도 WebGL account active plan 없이 이 checklist 기준을 따른다.
WebGL build와 smoke evidence artifact 소유는 별도 active plan이 열릴 때 그 plan으로 넘기고, 현재는 smoke 기록에 evidence 경로를 남긴다.

목적은 깊은 테스트가 아니라, 배포 직전 가장 중요한 사용자 여정이 **브라우저에서 끊기지 않는지** 빠르게 확인하는 것이다.

## Active Owner Boundary

- 이 문서는 현재 `WebGL Account/Garage / Product Smoke` residual의 checklist owner다.
- evidence 디렉터리, dated smoke log, multi-session fix plan이 필요해지면 새 active plan을 열고 이 문서는 reference checklist로만 내린다.
- WebGL smoke가 실행 불가하면 이 문서에 긴 작업 로그를 쌓지 말고 `blocked` 이유, 필요한 환경, 다음 owner만 짧게 남긴다.
...
- WebGL Account/Garage product smoke 상태 변경은 이 문서의 실기 결과를 기준으로 한다.
...
아래 형식으로 이 checklist 또는 이후 열리는 관련 계획 문서에 남긴다.
...
## 7. Smoke D — Settings/accessibility WebGL

목표:
브라우저에서 settings/accessibility UI가 열리고, 변경한 설정이 reload 후에도 유지되는지 확인한다.

### 절차

1. 최신 WebGL 빌드를 실행한다.
2. settings/accessibility entry를 연다.
3. 사운드, 표시, 또는 접근성 토글 중 저장되는 항목 하나 이상을 변경한다.
4. 앱을 새로고침한다.
5. 같은 계정/세션에서 변경값이 유지되는지 확인한다.

### 기대 결과

- settings/accessibility UI가 WebGL에서 입력 가능하다.
- 변경값 저장 피드백이 보인다.
- reload 후 변경값이 유지된다.

### 실패 시 기록

- 열리지 않는 UI entry 또는 입력 불가 요소
- 저장 실패 메시지
- reload 후 실제 값
- 브라우저 콘솔 에러
...
아래 네 항목이 각각 최소 1회 이상 성공 또는 실패 원인 명확화 상태가 되면, WebGL 실기 검증은 "막연한 TODO"에서 "관리 가능한 작업"으로 내려온다.

- `Garage save/load WebGL smoke`
- `Account delete WebGL smoke`
- `Google linking WebGL smoke`
- `Settings/accessibility WebGL smoke`
