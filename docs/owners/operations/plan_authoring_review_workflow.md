...
- `docs-rule-skill`: 문서 운영, 규칙, owner route, repo-local skill trigger 변경. `ops.document-management-workflow`, `ops.cohesion-coupling-policy`, 관련 owner skill을 먼저 보고, skill route/trigger 변경이면 `ops.skill-routing-registry`와 `ops.skill-trigger-matrix`도 확인한다.
