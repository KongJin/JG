...
- Governance surface 변경 여부는 다음 기준으로 판단한다:
  - **governance surface**: `AGENTS.md`, `docs/index.md`, `docs/owners/operations/*`, `docs/owners/ui-workflow/*`, `.codex/skills/*`, `tools/docs-lint/*`, `tools/rule-harness/*`, `.githooks/*`, `.github/workflows/docs-lint.yml`
  - **예시 (shard 사용)**: docs-lint hard-fail error 수정, skill trigger matrix fixture 추가/삭제, IMPORT_MANIFEST 등록, rule-harness config 변경
  - **예시 (shard 미사용)**: `docs/plans/*` 내용 갱신, `docs/owners/design/*` 판단 수정, `docs/owners/validation/*` checklist 보정, 일반 `tools/*/README.md` 설명 추가
  - **반례**: closeout artifact 자체를 수정해서 coverage를 맞추는 것은 shard 사용이나, shard에 포함되지 않은 rules-only file을 변경한 것은 residual로 남김
...
- HITL gate로 막혀 있는 active plan의 메타데이터에는 다음 필드를 추가할 수 있다:
  - `age`: 마지막 업데이트와 현재 날짜 사이의 기간 (예: `> age: 7 days`, `> age: 30 days`)
  - `> blocked owner`: 현재 plan을 막고 있는 owner (예: `> blocked owner: Unity manual audition verdict owner`)
  - `> next decision`: HITL gate를 통과한 후 다음 단계에서 해야 할 결정 (예: `> next decision: run 12 SFX auditions and record verdicts`)
  - 이 필드는 plan 맨 위 메타데이터 섹션에 `> key: value` 형식으로 추가한다. `> 상태` 필드는 `active`로 유지한다.
...
- `rules:sync-closeout` 실행 후 `rules:lint`에서 `recurrence-closeout-missing-changed-path` 또는 `missing-recurrence-closeout-coverage`가 발생하면 shard와 lint expectation 사이의 정합성이 깨진 상태다. 이때는:
  1. shard의 `changedPaths`에 현재 작업의 rules-only 파일이 모두 포함됐는지 확인한다.
  2. lint가 요구하는 coverage 문구가 `observedMutationClass`, `rootCause`, 또는 `prevention` 필드 중 하나에 포함됐는지 확인한다.
  3. 문제가 있으면 `npm run --silent rules:sync-closeout -- --artifact <shard_path>`로 shard를 재동기화한다.
  4. 병렬 작업 중 dirty shard가 있으면 별도 shard로 분리하는 것을 우선한다.
