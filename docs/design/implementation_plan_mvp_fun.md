...
페르소나 토의 샘플 합의([discussion_game_fun_personas.md](discussion_game_fun_personas.md) `## 샘플 토의 전문`)와 [game_design.md](../../agent/game_design.md) §「MVP 검증 포인트」「이번 분기 측정 정의」를 **구현·데이터·플레이테스트 작업**으로 나눈 문서다.

## SSOT 위임 (필수)

| 주제 | 단일 근거 |
|------|-----------|
| 수치·통과 기준·MVP 문장·웨이브 시간표 | [game_design.md](../../agent/game_design.md) 만 수정 |
...
4. **② 측정(새 스킬 vs 강화)** — `[MvpReward]` 로그·관찰로 [game_design.md](../../agent/game_design.md) 285–290행 절차 수행.

**프렌들리 파이어·4인 밀도:** 정식 지표 밖. 질적 메모만 ([game_design.md](../../agent/game_design.md) 274–276행).
...
**목표:** [game_design.md](../../agent/game_design.md) 278–283행 측정을 수행할 수 있는 상태인지 확인한다.
...
- [ ] [DefaultWaveTable.asset](../../Assets/Resources/Wave/DefaultWaveTable.asset): `WaveTableData`의 `waves` 배열을 [game_design.md](../../agent/game_design.md) **5웨이브·시간 배분**에 맞게 확장하는 것을 목표로 엔트리 추가·`enemyData`·`count`·`spawnRadius` 등 조정 (구체 수치는 SSOT 문서와 에디터에서 확정).
...
**목표:** [game_design.md](../../agent/game_design.md) 285–290행 측정 A/B 수행.
...
- Phase 2(시너지·메타·런 연장 등)는 [game_design.md](../../agent/game_design.md) Phase 2 절 범위 밖에서 확장하지 않는다.
