...
이 문서는 `Garage` Feature에 있던 `Unit` 도메인을 독립적인 `Unit` Feature로 분리한 설계 기록이다.
...
분리 전에는 `Garage/Domain/Unit.cs`가 Garage 안에 갇혀 있어서 Player/Enemy/Combat가 소비하려면 `Features.Garage.Domain`을 참조해야 했다. 이는 도메인 경계를 모호하게 만든 문제였다.
