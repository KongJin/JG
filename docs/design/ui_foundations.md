...
| `accent/orange` | `AccentOrange` | 상단(무장) 강조 |
| `accent/green` | `AccentGreen` | 하단(기동) 강조 |
...
- shared nav contract root는 UITK `UIDocument` 아래 `LobbyGarageNavBar` VisualElement다.
- shared nav는 `LobbyTabButton`, `GarageTabButton` 두 버튼만 가진다.
- Lobby/Garage surface root는 page-routing chrome을 직접 소유하지 않는다.
- `GarageMobileTabBar`는 page nav가 아니라 `하단(기동) / 중단(프레임) / 상단(무장)` 포커스 바로 유지한다.
...
- 현재 runtime `LobbyGarageNavBar`의 두 버튼 VisualElement 계약을 바꾸는 작업은 별도 runtime replacement pass로 분리한다.
...
- `하단(기동) / 중단(프레임) / 상단(무장)` 포커스 바는 현재 편집 부위를 바꾸는 컨트롤이다
