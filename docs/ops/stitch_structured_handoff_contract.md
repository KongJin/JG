...
> artifacts: `.stitch/contracts/screens/*.json`, `.stitch/contracts/mappings/*.json`, `.stitch/contracts/presentations/*.json`, `.stitch/contracts/schema/*.json`, `.stitch/contracts/components/*.json`, `.stitch/contracts/components/schema/*.json`

이 문서는 JG의 활성 `Stitch -> Unity` handoff 형식을 고정한다.
현재 활성 경로는 `source freeze -> screen manifest + unity-map + source-derived presentation contract`이고, generator는 `manifest semantic order -> binding check -> source-derived presentation apply -> controller wiring` 흐름만 허용한다.
지원되는 화면 구조에서는 이 계약이 실행 직전에 source에서 다시 준비될 수 있지만, 계약 구조 자체는 여기 정의한 형식을 그대로 따른다.
