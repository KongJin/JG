...
> artifacts: `in-memory://compiled/*/screen-manifest`, `in-memory://compiled/*/unity-map`, `in-memory://compiled/*/presentation-contract`, `.stitch/contracts/schema/*.json`, `.stitch/contracts/components/*.json`, `.stitch/contracts/components/schema/*.json`

이 문서는 JG의 활성 `Stitch -> Unity` handoff 형식을 고정한다.
현재 활성 경로는 `source freeze -> in-memory screen manifest + unity-map + source-derived presentation contract`이고, generator는 `manifest semantic order -> binding check -> source-derived presentation apply -> controller wiring` 흐름만 허용한다.
지원되는 화면 구조에서는 이 계약이 실행 직전에 source에서 다시 준비되며, 계약 구조 자체는 여기 정의한 형식을 그대로 따른다.
...
- `in-memory://compiled/*/screen-manifest`
  - Stitch source에서 확정된 semantic block 계약
- `in-memory://compiled/*/unity-map`
  - semantic block을 Unity target/path에 연결하는 binding
- `in-memory://compiled/*/presentation-contract`
  - source freeze에서 추출한 presentation 계약
- `.stitch/contracts/schema/stitch-screen-manifest.schema.json`
- `.stitch/contracts/schema/stitch-unity-map.schema.json`
- `.stitch/contracts/schema/stitch-presentation-contract.schema.json`
...
- per-surface files under `.stitch/contracts/screens/`
- per-surface files under `.stitch/contracts/mappings/`
- per-surface files under `.stitch/contracts/presentations/`
- historical contract paths under `.stitch/contracts/`
...
`presentation-contract`는 source freeze에서 실제로 읽어낸 시각 적용 계약이다.
semantic meaning이나 Unity binding을 다시 정의하지 않고, translation-ready presentation 값만 가진다.
...
`extractionStatus` 허용값:

- `pending-source-derivation`
- `resolved`
- `historical`

`sourceRefs`는 최소 아래를 가진다.

- `imagePath`
- `htmlPath`

`derivedFrom`은 사람이 source를 다시 추적할 때 필요한 source-derived 메모만 가진다.
예:
...
`unresolvedDerivedFields`는 아직 source에서 닫지 못한 필드를 기록한다.
active translation은 이 목록과 `extractionStatus`를 함께 보고 실행 가능 여부를 판단한다.

각 `elements[]` item은 최소 아래 필드를 가진다.

- `path`
- optional `role`
- optional `components`
- optional `siblingIndex`
- optional `rect`
- optional `properties`
- optional `notes`
...
`presentation-contract`는 source가 실제로 말해준 시각 값만 담고, translator가 새 UI 결정을 대신하게 만들지 않는다.

## 금지 입력

아래는 활성 handoff에 다시 넣지 않는다.

- historical override 문법
- historical contract ref path
- set별 handoff md 의존
- `png/html export를 직접 handoff 입력으로 삼는 규칙`
- `component-catalog를 manifest/map 대신 generator 직접 입력처럼 취급하는 규칙`
- source 확인 없이 손으로 적은 presentation literal
- source 확인 없이 손으로 적은 profile literal을 handoff owner처럼 쓰는 규칙
- translator fallback이나 debug 보정값을 presentation contract의 source-derived truth처럼 취급하는 규칙
- `pending-source-derivation` 상태를 active translation-ready contract처럼 쓰는 규칙

Stitch 원본 html/png는 source artifact일 뿐이고, generator 직접 입력은 아니다.
지원되는 화면 구조에서는 per-surface contract file을 repo에 남기지 않고, 동일 구조를 in-memory execution contract로만 준비한다.
...
- source freeze에서 확인한 값만 남긴다.
- `sourceRefs` 없이 presentation 값을 적지 않는다.
- 사람이 임의로 만든 literal을 source-derived 값처럼 쓰지 않는다.
- `derivedFrom`은 source provenance 설명만 남기고 새로운 runtime truth를 만들지 않는다.
- `unresolvedDerivedFields`가 남아 있으면 `extractionStatus = resolved`로 올리지 않는다.
- presentation contract는 target asset path, hostPath, controller wiring을 소유하지 않는다.
- missing value를 translator fallback으로 메우지 말고 contract를 다시 추출한다.
...
1. 필요하면 source를 읽는 내부 생성 단계를 먼저 수행한다.
2. manifest를 읽고 semantic block 순서를 확정한다.
3. contract completeness를 검증한다.
4. unity-map을 읽고 wiring 경로를 확인한다.
5. presentation contract가 `resolved`인지 확인한다.
6. contract에 적힌 값만 적용한다.
7. controller/view serialized reference를 연결한다.
8. preflight / translation / pipeline artifact를 남긴다.

generator가 historical skeleton이나 override 문법을 다시 해석하는 경로는 활성 기준이 아니다.
v1에서 generator가 `component-catalog`를 읽는 경로도 아직 활성 기준이 아니다.
script-side constants나 fallback으로 계약 누락을 메우는 경로도 활성 기준이 아니다.
hand-authored presentation literal을 source-derived truth처럼 읽는 경로도 활성 기준이 아니다.
...
- presentation 값이 `presentation-contract`와 `sourceRefs` 기준으로 추적 가능하다.
- translation-ready surface라면 `extractionStatus = resolved`다.
