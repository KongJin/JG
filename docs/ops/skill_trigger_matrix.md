...
| T08 | "JG scene/prefab authoring", "UITK/UI policy", "MCP evidence/runtime smoke"; generic "meta GUID/scene serialization/compile diagnostics" | `jg-unity-workflow`, `rule-unity` | `jg-coding-guardrails` for script edits; owner docs for acceptance | Do not mutate Unity surfaces in Plan Mode. |
