...
| T08 | "JG scene/prefab authoring", "UITK/UI policy", "MCP evidence/runtime smoke", "Stitch handoff Unity owner" | `jg-unity-workflow` | `rule-unity` only for generic Unity mechanics; `jg-coding-guardrails` for script edits | Do not mutate Unity surfaces in Plan Mode. |
